# =============================================================================
# routes/groups.py — API voor GROEPSCHATS: maken, berichten ophalen/sturen,
# en groepsberichten verwijderen.
# =============================================================================
...
def get_groups():
    """Geeft alle groepen waarin jij zit terug, gesorteerd op laatste activiteit
    (meest recente bovenaan)."""
    user_id, error, code = require_auth()
    if error:
        return error, code

    # Eerst de id's van alle groepen waar jij lid van bent.
    ...
    # Sorteer op tijd van het laatste bericht (of aanmaaktijd als er geen bericht is).
    payload.sort(
...
def create_group():
    """Maakt een nieuwe groep aan. Regels: minstens 2 vrienden, en je mag alleen
    echte (niet-geblokkeerde) vrienden toevoegen."""
    user_id, error, code = require_auth()
    if error:
        return error, code
    ...
    # Maak een nette lijst van unieke leden-id's (zonder jezelf, geen dubbels).
    member_ids = []
    ...
    # Controleer dat alle opgegeven gebruikers echt bestaan.
    ...
    # Controleer dat het allemaal vrienden zijn en niemand geblokkeerd is.
    ...
    # Geen naam opgegeven? Verzin er een op basis van de eerste paar leden.
    if not name:
    ...
    db.session.flush()  # flush() geeft de groep alvast een id, nog vóór commit

    # Voeg jezelf + alle gekozen leden toe aan de groep.
    ...
    # Verwittig alle leden realtime dat er een groep bij is.
...
def get_group_messages(group_id):
    """Haalt de berichten van een groep op. Mag alleen als je lid bent."""
    user_id, error, code = require_auth()
    if error:
        return error, code
...
def send_group_message(group_id):
    """Verstuurt een bericht (tekst of voice) in een groep. Mag alleen als lid."""
    user_id, error, code = require_auth()
    if error:
        return error, code
    ...
    # Stuur het bericht realtime naar ALLE groepsleden.
...
def delete_group_message(message_id):
    """Verwijdert een groepsbericht. Alleen de verzender mag dit, en alleen als
    hij/zij nog lid is van de groep."""
    user_id, error, code = require_auth()
    if error:
        return error, code
    ...
    # Verwittig alle leden dat een bericht verwijderd is.
